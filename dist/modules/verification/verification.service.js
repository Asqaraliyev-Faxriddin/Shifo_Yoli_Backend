...
let VerificationService = class VerificationService {
    ...
    async throwIFUserExist(email) {
        ...
        if (user) {
            throw new common_1.HttpException("Bu email dan foydalanuvchi foydalanmoqda.", common_1.HttpStatus.BAD_REQUEST);
        }
        return user;
    }
    ...
    async sendOtp(payload) {
        ...
        if (session) {
            throw new common_1.HttpException("Kode allaqachon yuborilgan", common_1.HttpStatus.BAD_REQUEST);
        }
        switch (type) {
        ...
        return { message: "Kode yuborildi" };
    }
    async verifyOtp(payload) {
        const { type, email, otp } = payload;
        const session = await this.redis.get(this.getKey(type, email));
        if (!session) {
            throw new common_1.HttpException("Kode muddati tugadi!", common_1.HttpStatus.BAD_REQUEST);
        }
        if (otp !== JSON.parse(session)) {
            throw new common_1.HttpException("Kode noto'g'ri!", common_1.HttpStatus.BAD_REQUEST);
        }
