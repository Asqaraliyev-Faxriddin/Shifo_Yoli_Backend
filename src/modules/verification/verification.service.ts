...
export class VerificationService {
    ...
    private async throwIFUserExist(email:string){
        ...
        if(user){
            throw new HttpException("Bu email dan foydalanuvchi foydalanmoqda.",HttpStatus.BAD_REQUEST)
        }
    ...
    async sendOtp(payload:SendOtpDto){
        ...
        if(session){
            throw new HttpException("Kode allaqachon yuborilgan",HttpStatus.BAD_REQUEST )
        }
    ...
    async verifyOtp(payload:VerifyOtpDto){
        ...
        if(!session){
            throw new HttpException("Kode muddati tugadi!",HttpStatus.BAD_REQUEST)
        }

        if(otp !==JSON.parse(session)){
            throw new HttpException("Kode noto'g'ri!",HttpStatus.BAD_REQUEST)
